...
class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
	...
	public void drawPyramid(GL2 gl2) {
		double radius = 3.0D;
		int corners = 8;
		float[] c1 = new float[]{1.0F, 2.0F, 1.0F}; 
		float[] c2 = new float[]{0.8F, 0.4F, 0.3F}; 
		...
		int i;
		for (i = 0; i < corners; ++i) {
			gl2.glVertex3f((float) (Math.cos((double) (i * 2) * Math.PI / (double) corners) * radius), bottomHeight,
					(float) (Math.sin((double) (i * 2) * Math.PI / (double) corners) * radius));
			gl2.glColor3fv(c1, 0);
			gl2.glVertex3f((float) (Math.cos((double) ((i + 1) * 2) * Math.PI / (double) corners) * radius), bottomHeight, 
					(float) (Math.sin((double) ((i + 1) * 2) * Math.PI / (double) corners) * radius));
			gl2.glColor3fv(c2, 0);
		}
	...
	public void drawCorkscrew(GL2 gl2) {
		gl2.glEnable(2832); 
		gl2.glColor3f(1.0F, 2.0F, 3.0F);
		float corners = 20.0F;
		float radius = 1.5F; 
		float uplift = -5.0F; 
		float pointSize = 4.0F; 
		int numberOfSpirals = 7; 
